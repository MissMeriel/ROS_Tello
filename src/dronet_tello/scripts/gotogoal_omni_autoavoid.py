...
def main():
	...
	print("GOAL FROM ARGS: "+str(sys.argv))
	time.sleep(2)
	...
	while not rospy.is_shutdown():
		...
		print("global angle_drone_to_goal: "+str(math.degrees(math.atan2(final_goal_y-curr_y, final_goal_x-curr_x))))
		...
		avoid = obstacle_in_path and distance_drone_to_obstacle < obstacle_threshold and avoid_count < 5
		avoids.append(avoid)
		...
		if (distance_to_final_goal < threshold):
			...
			goal_y = curr_y + 5 * math.sin(avoid_angle)
			
			angle_drone_to_goal = math.atan2(goal_y-curr_y, goal_x-curr_x)-curr_angle

			vel.linear.x = math.cos(angle_drone_to_goal) * dt
			vel.linear.y = math.sin(angle_drone_to_goal) * dt
