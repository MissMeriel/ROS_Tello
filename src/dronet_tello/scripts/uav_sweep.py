...
def process_sysargs():
	...
	distance_1 = math.sqrt((curr_x-goal_x[0])**2+(curr_y-goal_y[0])**2+(curr_y-goal_y[0])**2)
...
def main():
	...
	velocity_publisher = rospy.Publisher("/velocity", Bool, queue_size=1)
	...
	while not rospy.is_shutdown():
		...
		#reached sweep area
		if (distance_to_goal < threshold and interpolation_x[goal_counter] == goal_x[0] and interpolation_x[goal_counter] == goal_y[0]):
			vel.linear.x = 0
			vel.linear.y = 0
			print("GOAL REACHED within threshold: " + str(distance_to_goal))
			state_publisher.publish("SWEEP AREA REACHED")
			hover_count += dt
		...
		#finished sweeping
		elif (distance_to_goal < threshold and interpolation_x[goal_counter] == goal_x[1] and interpolation_x[goal_counter] == goal_y[1]):
