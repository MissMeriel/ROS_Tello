...
def main():
	...
	while(True):
		...
		if (distance_to_goal < threshold):
			...
			if(obstacle_in_path and not avoid):
				...
				if(distance_to_obstacle < threshold):
					...
					goal_y = obs_y + math.sin(angle_obs_to_goal) * 10
				...
				angle_drone_to_goal = angle_drone_to_goal + threshold * 2
				#obs radius + distance threshold *2
				goal_x = 3 * math.cos(angle_drone_to_goal)
				goal_y = 3 * math.sin(angle_drone_to_goal)
				#if(curr_angle < 0):
